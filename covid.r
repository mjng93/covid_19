...
covid=merge(covid,as.data.frame(group_by(covid, Province.State) %>% filter(Confirmed>=10) %>% summarise(start.c.100=min(ObservationDate))),by="Province.State",all=TRUE) # add date of 100th case
covid=merge(covid,as.data.frame(group_by(covid, Province.State) %>% filter(Deaths>=1) %>% summarise(start.d.10=min(ObservationDate))),by="Province.State",all=TRUE) #add date of 10th death
